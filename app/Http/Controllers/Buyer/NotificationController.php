...
class NotificationController extends Controller
{
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('buyer.notifications.index')->with('success', 'Notification deleted successfully.');
    }
    ...
    public function dropdown()
    {
        ...
        foreach ($notifications as $notification) {
            ...
            if ($notification->type === 'payment_approved' && isset($notification->data['payment_id'])) {
                $messageHtml = "<a href=\"/buyer/payments\" class=\"text-sm text-blue-600 hover:text-blue-800\">{$notification->message}</a>";
            } elseif ($notification->type === 'property_approved' && isset($notification->data['property_id'])) {
                $messageHtml = "<a href=\"/buyer/my-properties/{$notification->data['property_id']}\" class=\"text-sm text-blue-600 hover:text-blue-800\">{$notification->message}</a>";
            } else {
