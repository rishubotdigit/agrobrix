...
<div class="mb-12">
    <div class="flex justify-between items-center mb-4">
        <div class="flex items-center gap-4">
            ...
            @if($viewMode == 'grid' && $properties->count() > 0)
            <div class="flex items-center bg-gray-100 px-3 py-1 rounded-full">
                <input type="checkbox" id="globalSelectAll" class="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary cursor-pointer" onchange="toggleSelectAll(this)">
                <label for="globalSelectAll" class="ml-2 text-sm font-medium text-gray-600 cursor-pointer">Select All</label>
            </div>
            @endif
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300 relative group">
            <!-- Checkbox -->
            <div class="absolute top-2 left-2 z-10">
                <input type="checkbox" value="{{ $property->id }}" class="property-checkbox w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary shadow-sm cursor-pointer" onchange="updateBulkState()">
            </div>
            ...
            <!-- Property Details -->
            <div class="p-4">
                <h3 class="text-sm font-semibold text-gray-900 mb-1 overflow-hidden h-10 leading-snug" style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">{{ $property->title }}</h3>
                ...
                <div class="flex items-center text-xs text-gray-500 mb-2">
                    ...
                    {{ Str::limit($property->area ?? 'N/A', 20) }}
                </div>

                <div class="flex items-end justify-between mb-3 border-b border-gray-100 pb-3">
                    <div class="text-lg font-bold text-primary">₹{{ number_format($property->price) }}</div>
                    @if($property->plot_area)
                        <div class="text-xs text-gray-500">{{ $property->plot_area }} {{ $property->plot_area_unit }}</div>
                    @endif
                </div>
                ...
                <div class="flex items-center text-xs text-gray-500 mb-3">
                    ...
                    <span class="truncate">{{ $property->owner->name }}</span>
                </div>
                ...
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-4">
                            <input type="checkbox" id="selectAll" class="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary" onchange="toggleSelectAll(this)">
                        </th>
...
function toggleSelectAll(source) {
    ...
    checkboxes.forEach(cb => cb.checked = source ? source.checked : false);
    
    // Sync other select all checkboxes if any exist (e.g. table header vs global)
    if (source) {
        document.querySelectorAll('#selectAll, #globalSelectAll').forEach(el => {
