            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    ...
                    @if($notifications->where('seen', false)->count() > 0)
                        <form method="POST" action="{{ route('buyer.notifications.markAllAsRead') }}" class="inline">
                            @csrf
                                ...
                                <div class="flex justify-between items-start">
                                    <div class="flex-1">
                                        ...
                                        @if($notification->type === 'payment_approved' && isset($notification->data['payment_id']))
                                            <a href="{{ route('buyer.plan-purchases.index') }}" class="text-blue-600 hover:text-blue-800">{{ $notification->message }}</a>
                                        @elseif($notification->type === 'property_approved' && isset($notification->data['property_id']))
                                            <a href="{{ route('buyer.properties', ['property' => $notification->data['property_id']]) }}" class="text-blue-600 hover:text-blue-800">{{ $notification->message }}</a>
                                        @else
                                    ...
                                    <div class="flex space-x-2">
                                        @if(!$notification->seen)
                                            <form method="POST" action="{{ route('buyer.notifications.markAsRead', $notification->id) }}" class="inline">
                                                @csrf
                                        ...
                                        @endif
                                        <form method="POST" action="{{ route('buyer.notifications.delete', $notification->id) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this notification?')">
                                            @csrf
                                            @method('DELETE')
