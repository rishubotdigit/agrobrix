<div id="sidebar" class="w-64 gradient-bg text-white fixed top-0 left-0 z-50 transform -translate-x-full transition-transform duration-300 ease-in-out md:static md:translate-x-0 md:block shadow-lg">
    ...
    <nav class="mt-6">
        ...
        <a href="{{ route('admin.reports.index') }}" class="flex items-center px-6 py-3 hover:bg-emerald-700 transition-colors {{ request()->routeIs('admin.reports.*') ? 'bg-emerald-700 border-r-4 border-white' : '' }}">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
            </svg>
            Reports
        </a>
        <a href="{{ route('admin.wishlist.index') }}" class="flex items-center px-6 py-3 hover:bg-emerald-700 transition-colors {{ request()->routeIs('admin.wishlist.*') ? 'bg-emerald-700 border-r-4 border-white' : '' }}">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div>
            <button id="inquiries-toggle" class="flex items-center w-full px-6 py-3 hover:bg-emerald-700 transition-colors {{ request()->routeIs(['admin.leads.*', 'admin.viewed-contacts.*']) ? 'bg-emerald-700 border-r-4 border-white' : '' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                Inquiries
                <svg class="w-4 h-4 ml-auto transform transition-transform" id="inquiries-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                </svg>
            </button>
            <div id="inquiries-submenu" class="hidden ml-6">
                <a href="{{ route('admin.leads.index') }}" class="flex items-center px-6 py-2 hover:bg-emerald-700 transition-colors {{ request()->routeIs('admin.leads.*') ? 'bg-emerald-700 border-r-4 border-white' : '' }}">
                    Leads
                </a>
                <a href="{{ route('admin.viewed-contacts.index') }}" class="flex items-center px-6 py-2 hover:bg-emerald-700 transition-colors {{ request()->routeIs('admin.viewed-contacts.*') ? 'bg-emerald-700 border-r-4 border-white' : '' }}">
                    Viewed Contacts
                </a>
            </div>
        </div>
