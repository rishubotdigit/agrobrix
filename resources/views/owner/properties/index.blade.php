...
<!-- Add Property Button -->
<div class="mb-6">
    <a href="{{ route('owner.properties.create') }}" class="bg-primary text-white px-6 py-3 rounded-lg font-medium hover:bg-emerald-700 transition-colors inline-flex items-center {{ $usage['current_listings'] >= $usage['max_listings'] ? 'opacity-50 cursor-not-allowed pointer-events-none' : '' }}">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300">
        ...
        <div class="p-6">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('owner.properties.show', $property) }}" class="flex-1 bg-gray-100 text-gray-700 px-4 py-2 rounded-lg font-medium hover:bg-gray-200 transition-colors text-center text-sm">
                    View
                </a>
                <a href="{{ route('owner.properties.edit', $property) }}" class="flex-1 bg-blue-100 text-blue-700 px-4 py-2 rounded-lg font-medium hover:bg-blue-200 transition-colors text-center text-sm">
                    Edit
                </a>
...
function featureProperty(propertyId) {
    if (confirm('Are you sure you want to feature this property?')) {
        fetch(`/owner/properties/${propertyId}/feature`, {
            method: 'POST',
            headers: {
...
function unfeatureProperty(propertyId) {
    if (confirm('Are you sure you want to unfeature this property?')) {
        fetch(`/owner/properties/${propertyId}/unfeature`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
